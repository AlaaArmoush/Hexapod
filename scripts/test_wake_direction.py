...
# direction → (action_description, commands_to_send)
_DIRECTION_MAP = {
    "left":        ("camera_pan left",       [lambda: build_camera_pan("left")]),
    "front_left":  ("camera_pan front_left", [lambda: build_camera_pan("front_left")]),
    "front":       ("camera_pan center",     [lambda: build_camera_pan("center")]),
    "front_right": ("camera_pan front_right",[lambda: build_camera_pan("front_right")]),
    "right":       ("camera_pan right",      [lambda: build_camera_pan("right")]),
    "back":        ("rotate 180° + center",  [lambda: build_rotate(dir="left", degrees=180),
                                              lambda: build_camera_pan("center")]),
    "center":      ("camera_pan center",     [lambda: build_camera_pan("center")]),
}


def make_callback(bridge, last_time: list[float], dry_run: bool):
    def on_wakeword(model_name: str, score: float, direction: str) -> None:
        ...
        label, cmds = _DIRECTION_MAP.get(direction, _DIRECTION_MAP["center"])
        print(f"\n[test] wake word detected  score={score:.3f}  direction={direction!r}  → {label}",
              file=sys.stderr)

        if dry_run:
            for cmd_fn in cmds:
                print(f"[test] DRY-RUN: {cmd_fn()}", file=sys.stderr)
        else:
            for cmd_fn in cmds:
                try:
                    bridge.send_command(cmd_fn())
...
def main() -> None:
    ...
    detector = WakeWordDetector(
        ...
        on_wakeword=make_callback(bridge, last_time, dry_run),
